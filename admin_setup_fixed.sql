-- Admin Whitelist Table Setup - FIXED VERSION
...
DROP POLICY IF EXISTS "Anyone can check admin whitelist for eligibility" ON admin_whitelist;
DROP POLICY IF EXISTS "Admins can view full admin whitelist" ON admin_whitelist;
DROP POLICY IF EXISTS "Admins can add to admin whitelist" ON admin_whitelist;
DROP POLICY IF EXISTS "Admins can update admin whitelist" ON admin_whitelist;
DROP POLICY IF EXISTS "Admins can delete from admin whitelist" ON admin_whitelist;
...
-- Create RLS policies
-- Allow anyone to check if an email is in the whitelist (needed for signup eligibility)
CREATE POLICY "Allow eligibility check" ON admin_whitelist
